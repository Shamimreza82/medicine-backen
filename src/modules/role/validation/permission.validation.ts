...
const permissionNames = authPermissions.map((item) => item.name) as [string, ...string[]];

export const assignPermissionsSchema = z.object({
  body: z.object({
    permissions: z.array(z.enum(permissionNames)).min(1, 'At least one permission is required'),
  }),
});
