...
const createRoleService = async (payload: TCreateRoleInput, user: TJwtPayload) => {
  const { tenantId } = user;
  const { slug } = payload;

  // Check existing role
  const existingRole = await RoleRepository.getTenantByTenantIdWithSlug(tenantId, slug);

  if (existingRole) {
    throw new AppError(StatusCodes.CONFLICT, ROLE_MESSAGES.ALREADY_EXISTS);
  }
