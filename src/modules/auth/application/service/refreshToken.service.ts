...
const refreshTokenService = async (token: string) => {
  ...
  if (
    (isExistUser && isExistUser?.status === 'INACTIVE') ||
    isExistUser?.status === 'LOCKED' ||
    isExistUser?.status === 'SUSPENDED'
  ) {
    throw new AppError(
      StatusCodes.INSUFFICIENT_SPACE_ON_RESOURCE,
      `Account temporarily ${isExistUser.status}, please contect admin`,
    );
  }
